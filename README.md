  ...
  build:
    ...
    steps:
      ...
      - name: Trigger model training notebook from PR branch
        uses: step-security/run-notebook@v0
        with:
...
to each `step-security/run-notebook` step to trigger notebook execution against different workspaces.
